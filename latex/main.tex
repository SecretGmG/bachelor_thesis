...
\begin{abstract}
    ...
    In this thesis we explore the consequences of this complexification of propagator masses in the context of the numerical computation of loop integrals in momentum-space using Cross-Free Family representation of the triangle integral. In particular, the regularization of threshold singularities needs to be generalized in this case since the threshold loci lie in the complex plane.
...
The nontrivial contribution of higher-order contributions are often dominated by loop integrals, which are harder to compute with existing analytical methods like integration by parts~\cite{laporta_high-precision_2000} or differential equation systems for master integrals~\cite{henn_multiloop_2013}.
...
Performing the integral in momentum-space often facilitates its interpretation in terms of the relevant physics, mostly because of the energy scales at play, as well as singularity analysis is simplified in this context. Moreover, observables of interests for collider phenomenology naturally take particle momenta as arguments, rendering momentum-space integration most suitable to yield such differential predictions

Contour deformation~\cite{capatti_numerical_2020} is a method where the integral contour is deformed into an analytical continuation of the integration domain. It is a technique that regularizes integrable singularities the by deforming the integration contour in the complex plane. In this deformation one must be careful not to introduce additional residues by deforming the integration over other poles which previously did not contribute to the integral.
...
It shows that the Cross-Free Family (CFF) representation can be obtained by recursively summing over all possible acyclic edge orientations of the given graph. In the case of the triangle, there are six possible acyclic edge orientations $\mathcal{S} = {\{+,-\}}^3 \setminus
...
To make further progress we will assume that the condition in ref.~\eqref{eq:e_surface_existence_condition} holds and proceed by noticing that the E-surface is of the same structure as the following.
\begin{equation}
...
\begin{itemize}
    \item \textbf{Numerical check existence condition:}\\
          ...
          perform this check per sample. This means that samples concerning the same E-surface may be excluded / included depending on the solid angle $\hat{\vb{k}}$.
...
In sect.~\ref{sec:e_surface} we already showed how the find the zeros of the E-surface in the radial variable. Let us assume a fixed radial direction $\hat{\vb{k}}$ and denote the set of solutions as $\mathcal{K}_{ij}$ and the solutions themselves as $k^* \in \mathcal{K}_{ij}$.
...
Without loss of generality we may reorder the momenta, such that the following holds. $\vb{q}_i$ such that $\vb{q}_1 < \vb{q}_2 < \vb{q}_3$. With this ordering the existence condition derived in eq.~\eqref{eq:e_surface_existence_condition} is fulfilled exactly for the E-surfaces $\ELIPT{i}{j}$ with $i<j$. Thus we need to consider the counterterms of the E-surfaces corresponding with the indices $\mathcal{E} = \{(1,2),(2,3),(1,3)\}$.
...
This corresponds almost exactly to the structure of the single counterterm from eq.~\eqref{eq:single_counterterm}. The counterterm removing the pole at $k^*\in \mathcal{K}_{12}$ is therefore given by
\begin{equation}
...
The source code of our implementation is available on \emph{GitHub}\footnote{\url{https://github.com/SecretGmG/triangler}} under the MIT license. It contains jupyter notebooks which can be used to run all of the examples presented in sect.~\ref{sec:results}.
...
The grid starts out uniformly, but then adapts to concentrate around the peaks. Since each bin is sampled with equal probability, regions where the bins are smaller will be sampled more frequently. This drastically improves the convergence of the algorithm and lowers the error.
