...
Existing applications, however, predominantly focus on integrals with real internal masses~\cite{kermanschah_numerical_2022,kermanschah_numerical_2024,capatti_loop_2019}. In practice there are many situations where complex internal masses are justified. Given that the integral is defined along the real axis, the imaginary part of the pole already acts as a regulator of the threshold singularity by shifting the singularity to the complex plane. This is analogous to a finite version of the Feynman causal prescription $+ i \varepsilon$. In practice, such regulators can prove to yield integrals of very large variances as the regularization parameter $\varepsilon$ gets small in the narrow-width limit.
...
\ifmore
In Appendix~\ref{app:anomalous_thresholds}, we present a qualitative discussion of the geometrical properties of E-surfaces regarding anomalous thresholds. We find that anomalous threshold coincide with configurations where E-surfaces touch.

In Appendix~\ref{app:generalization_to_higher_loop_count}, we present a qualitative discussion on the generalization og the method to higher loop count integrals. We investigate the feasibility of finding the complex-valued thresholds using to hyper-hemispherical coordinates and Newtons method.
\fi
...
The structure of the singularities of the triangle integral~\eqref{eq:triangle_integral} is intricate, and the singularities are present even for large momenta $\vb{k}^\mu \to \infty$. This can be illustrated by considering eq.~\eqref{eq:triangle_integral} in one spatial dimension. The integrand can then be visualized for all values of $(\vb{k}^0, \vb{k^1}) \in
...
\end{align}
Inserting the results from eq.~\eqref{eq:surfaces} and eq.~\eqref{eq:residue} into eq.~\eqref{eq:cauchy} we get the LTD expression.
...
\begin{itemize}
    \item \textbf{Numerical check subtraction condition:}\\
    ...
    \item \textbf{Minimum value subtraction condition:}\\
...
In recent approaches, only singularities lying exactly in the integration domain are considered for threshold subtraction~\cite{kermanschah_numerical_2022,kermanschah_numerical_2024,capatti_loop_2019}. In this thesis we extend this idea to singularities that are not exactly in the integration domain, but instead are shifted to the complex plane. 

While not strictly necessary for the convergence of the integrand, this extension allows for a more general treatment of singularities and improves numerical stability in cases where the singularities are very close to the integration domain.
...
\end{equation}
We can now define a counterterm that will cancel the singularity at $k^*$. This is done by using the Taylor expansion of the integrand to the first order multiplied by a mask function $\chi(\va{k})$.
\begin{equation}
...
\end{equation}
Note that the due to the branch cut in the complex log, which is chosen to be on the negative real axis, the result can depend on the prescription. We write the prescription implicit in the variable $k^*$, whose prescription follows from eq.~\eqref{eq:quad_k_solutions}.
...
\begin{itemize}
    ...
    The subtraction region is a sliver with a half-width of $\lambda_0$ centered around the threshold located in the complex plane, meaning that the subtraction region viewed from the real axis gets smaller as the imaginary part of the threshold gets larger.
    \begin{align}
        \mu(k^*) &= \Re(k^*)\\
...
\begin{equation}
    q^0_1 < q^0_2 < q^0_3
\end{equation}
With this ordering the existence condition derived in eq.~\eqref{eq:e_surface_existence_condition} is fulfilled exactly for the E-surfaces $\ELIPT{i}{j}$ with $i<j$. We will therefore only consider counterterms for the E-surfaces that fulfill this condition.

We will now construct the counterterm for such an E-surface $\ELIPT{i}{j}$ with $i<j$. For this we need to factor out the term $\ELIPT{i}{j}$ from the CFF representation of the triangle integral. The factored expression is given by
\begin{equation}
    \frac{1}{\ELIPT{i}{j}} \frac{1}{{(4\pi)}^3}\frac{1}{E_1E_2E_3} \qty(\sum_{(l,m) \in \mathcal{C}(i,j)} \frac{1}{\ELIPT{l}{m}}) = \frac{1}{\ELIPT{i}{j}} \, f_{ij}(\va{k}) ,
\end{equation}
where
\begin{align}
...
There exists one notable difference between the counterterm here and the case discussed for a single E-surface, which may cause problems. For the single E-surface we assumed that the factor $f$ was bounded. This is clearly not the case for the factor $f_{ij}$, as it includes terms of the form $1/\ELIPT{l}{m}$ which can be unbounded. This means that the factor $f_{ij}$ itself may have singularities which would introduce divergences in the counterterm.
...
\begin{equation}
    \int_{-\infty}^{\infty} \dd{k} k^2 g(k) = 1
\end{equation}
...
\begin{equation}
    I =
    \int_{\mathcal{H}} \dd[2]{\hat{\vb{k}}} \int_{-\infty}^{\infty} \dd{k} k^2 \qty[ \mathbf{a}\;\mathcal{I}_{CFF} +
    \sum_{i<j} \sum_{k^* \in \mathcal{K}_{ij}}
...
\begin{itemize}
    \item Enable/disable the numerical check subtraction condition.
...
\end{itemize}
The minimum value subtraction condition is controlled via an additional hyperparameter which determines the maximum allowed value of $\min_{\vb{k}} \ELIPT{i}{j}$ for each E-surface. To enable the minimum value subtraction condition set the hyperparameter zero, to disable the minimum value subtraction condition set the hyperparameter to a very large value.

All other hyperparameters and parameters can be set via a context object which is used to call the compiled expression.

The source code of our implementation is available on \emph{GitHub}\footnote{\url{https://github.com/SecretGmG/triangler}}
...
\emph{OneLOop} implements the analytical solutions for scalar loop integrals with up to 4 legs, thus providing the exact results for the integrals we are interested in. To facilitate the comparison we have implemented a Python wrapper for the \emph{OneLOop} package called \emph{OneLOopBridge}\footnote{\url{https://github.com/SecretGmG/OneLOopBridge}} 
...
In this example all of the three possible E-surfaces exist, this can be easily checked by applying the minimum value subtraction condition derived in sect.~\ref{sec:extraneous_solutions}.
...
\begin{figure}[H]
    ...
    RSD for a range of masses comparing the numerical and minium value E-surface subtraction conditions.
\end{figure}

\subsection{Threshold masses}\label{sec:threshold_masses}

We will now take a closer look at what happens in configurations with real masses, where imaginary solutions to the E-surface eq.~\eqref{eq:e_surface} exist. We will check if subtracting the imaginary solutions in these cases improves numerical stability especially for configurations very close to thresholds where E-surfaces come into existence.
...
We will sweep the real-valued mass $m$ across a large range of values to check numerical stability of the integral as E-surfaces go out of existence. We will also investigate three different subtraction methods and compare the results. 
\begin{itemize}
    \item \textbf{No subtraction condition} No subtraction is performed.
    \item \textbf{Minimum value subtraction condition} The real part of the minimum value of the E-surface is used to determine if the threshold subtraction is needed. As an E-surface goes out of existence, the minimum value of the E-surface becomes positive and simply stops. This method is performed with a subtraction region with a fixed half-width centered around the \emph{real} part of the threshold location.
    \item \textbf{Numerical check subtraction condition} Only a numerical check is performed to determine if the threshold subtraction is needed. This method is performed with a subtraction region with a fixed half-width centered around the \emph{real} part of the threshold location. This means that complex valued solutions always get subtracted if they exist.
    \item \textbf{Sliver subtraction with complex distance} For this method the effective region of the subtraction is a sliver with a half width of $\lambda = \SI{1}{\GeV}$ centered around the \emph{complex} valued location of the thresholds, in accordance to eq.~\ref{eq:sliver_mod}. The mask for the integrated counterterm stays as a sliver with a fixed half width centered around the \emph{real} part of the threshold. As the imaginary part of the threshold gets larger, the subtraction region gets smaller, until it vanishes.
\end{itemize}
...
The result of the numerical integration is shown in fig.~\ref{fig:threshold_masses_integral}. It can be seen that all methods converge well on the correct result, except for the method where no subtraction is performed.
\begin{figure}[H]
    \centering
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/threshold_masses/global_check.png}
        \caption{With minimum value subtraction condition}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/threshold_masses/per_sample_check.png}
        \caption{Without minimum value subtraction condition}
    \end{subfigure}
        \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/threshold_masses/variable.png}
        \caption{Sliver subtraction with complex distance}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/threshold_masses/unsubtracted.png}
        \caption{Unsubtracted integrand}
    \end{subfigure}
    \caption{Integration result with different E-surface subtraction conditions}\label{fig:threshold_masses_integral}
\end{figure}

We now examine the RSD for the different methods. This is shown in fig.~\ref{fig:threshold_masses_rsd}. As expected the integration for masses smaller than the smallest threshold mass $m^2 < \SI{0.25}{\GeV^2}$ has exactly the same behavior for the numerical and minimum value check. More interesting is that we can see that even for larger masses, up to the largest threshold mass $m^2 < \SI{2}{\GeV^2}$, the RSD of the two methods are very close to each other. As the largest threshold mass $m^2 < \SI{2}{\GeV^2}$ is reached, the RSD for the numerical check starts to diverge from the minimum value method. Surprisingly it is the numerical check which subtracts all solutions that has the highest RSD\@, while the other methods outperform the numerical check. 

The fact that the complex distance method still performs well, indicates that the threshold subtraction of non-existing E-surfaces is only needed very close to the threshold masses. For configurations where the E-surface is not close to existing, subtraction actually hurts performance by introducing variance.
...
To investigate this further we plot the RSD for a smaller range of masses near $m^2 < \SI{2}{\GeV^2}$ This is shown in fig.~\ref{fig:threshold_masses_rsd_zoomed}. There it can be seen, that the RSD for the unsubtracted method starts to decrease even for masses smaller than the threshold mass. This indicates that as the size of the E-surface decreases, its effect on the integrand becomes less important. As the E-surface transitions from a 2-dimensional spheroidal surface to a 0-D point, the effect of the E-surface on the integrand gets smaller. Threshold subtraction can increase the RSD slightly by enlarging the integrand over broad regions, where it initially might have been close to zero. We also observe that the complex distance method outperforms all other methods throughout the entire range of masses, indicating that this is the most effective method to regularize the integrand.
\begin{figure}[H]
    \centering
    ...
    RSD for a range of masses comparing the numerical minium value E-surface subtraction conditions.
\end{figure}
...
\begin{figure}[H]
    ...
    RSD for a range of masses comparing the numerical and minimum value E-surface subtraction conditions.
\end{figure}
