...
We will sweep the real-valued mass $m$ across a large range of values to check numerical stability of the integral as E-surfaces go out of existence. In the case of the minimum value subtraction condition, the subtraction for these E-surfaces simply stops, while in the case of the numerical check the threshold subtraction is still applied to complex solutions. It will be interesting to see if the numerical check is more stable than the minimum value subtraction condition, especially near the threshold masses.
...
We now examine the RSD for the different methods. This is shown in fig.~\ref{fig:threshold_masses_rsd}. As expected the integration for masses smaller than the smallest threshold mass $m^2 < \SI{0.25}{\GeV^2}$ has exactly the same behavior for the numerical and minimum value check. More interesting is that we can see that even for larger masses, up to the largest threshold mass $m^2 < \SI{2}{\GeV^2}$, the RSD of the two methods are very close to each other. As the largest threshold mass $m^2 < \SI{2}{\GeV}^2$ is reached, the RSD for the numerical check starts to diverge from the minimum value method. Surprisingly it is the minimum value method that has the lower RSD\@.
