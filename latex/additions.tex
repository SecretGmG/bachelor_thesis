...
We now examine the integral near the intersection of two E-surfaces $A$ and $B$. Assume the surfaces intersect at the origin, and linearize each surface near this point. The integral then reduces locally to

\begin{equation}
    \int \dd[3]{k} \frac{1}{(\eta_A' \cdot \mathbf{k}) (\eta_B' \cdot \mathbf{k}) + i\epsilon},
\end{equation}

where $\eta_A'$ and $\eta_B'$ are the gradients of the surfaces at the intersection.

We distinguish two cases:
\begin{enumerate}
    \item \textbf{Parallel gradients} surfaces touch
          If $\eta_A' \parallel \eta_B'$, the singular planes coincide, and the integrand becomes

          \begin{equation}
              \frac{1}{{(\eta_A' \cdot \mathbf{k})}^2 + i\epsilon}.
          \end{equation}

          Choosing coordinates with $z$ along $\eta_A'$ and $x,y$ in the plane transverse to it, the integral scales as

          \begin{equation}
              \int dx,dy,dz \frac{1}{|\eta_A'|^2 z^2 + i\epsilon} \sim \order{\frac{i}{\epsilon}},
          \end{equation}

          giving a \emph{linear divergence} in $1/\epsilon$.

    \item \textbf{Non-parallel gradients} surfaces intersect along a line/ring
          If $\eta_A' \not\parallel \eta_B'$, the planes intersect along a line. In coordinates $u,v,\phi$ with $u$ along $\eta_A'$-transverse, $v$ along $\eta_B'$-transverse, and $\phi$ along the intersection line, the integrand locally behaves like
          \begin{equation}
              \frac{1}{(|\eta_A'| u)(|\eta_B'| v) + i\epsilon}.
          \end{equation}
          Integrating over the transverse directions gives a \emph{finite imaginary contribution}, scaling as
          \begin{equation}
              \int du,dv,d\phi \frac{1}{(|\eta_A'| u)(|\eta_B'| v) + i\epsilon} \sim \order{i},
          \end{equation}
\end{enumerate}
