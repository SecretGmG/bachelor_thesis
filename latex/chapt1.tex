...
Without loss of generality we will consider the case where $\va{k}_i = -\va{k}_j = \va{k}$, as it will make the following calculations less tedious.
\begin{equation}
    \ELIPT{i}{j} =  \sqrt{{(\va{k}-\va{q})}^2+m^2} + \sqrt{{(\va{k}+\va{q})}^2+m^2} + 2k^0 \overset{!}{=} 0
\end{equation}
Notice that the square roots of the E-Surface can be eliminated, because it is of the following form
\begin{align}
    \sqrt{A}+\sqrt{B} &= C\\
    2\sqrt{AB} &= C^2-A-B\\
    4AB &= (C^2-A-B)^2
...
The geometry of the solution is more apparent when we parameterize this spherically, with $\va{k} = k\,\hat{k} + \va{k}_0$ where $\norm*{\hat{k}} = 1$ and $k = \norm*{\va{k}}$.
\begin{align}
    %(\va{k}\cdot\va{q})^2-(\va{k}^2+\va{q}^2+m^2){q^0}^2+{q^0}^4 = 0\\
...
Then the real part of this integration is always zero because $\abs{\lambda-i\Delta} = \abs{-\lambda-i\Delta}$, we find.
\begin{equation}
